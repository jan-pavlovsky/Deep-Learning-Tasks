        ...
        def batches(self, size=None, image_processing=None):
            ...
            while len(permutation):
                ...
                for key in self._data:
                    if key == "images":
                        batch[key] = np.zeros(
                            [batch_size, Caltech42.MIN_SIZE, Caltech42.MIN_SIZE, Caltech42.C], dtype=np.float32)
                        for i, index in enumerate(batch_perm):
                            data = image_processing(
                                self._data[key][index]) if image_processing is not None else self._data[key][index]
                            if type(data) != np.ndarray:
                                raise ValueError(
                                    "Caltech42: Expecting images after `image_processing` to be Numpy `ndarray`")
                ...
                for name in sorted(caltech42_zip.namelist()):
                    ...
                    with caltech42_zip.open(name, "r") as image_file:
                        ...
                        if image_processing is not None:
                            data["images"][-1] = image_processing(
                                data["images"][-1])
