...
class Network:
    def __init__(self, args, num_source_chars, num_target_chars):
        class Model(tf.keras.Model):
            def __init__(self):
                ...
                # TODO source_embeddings as a masked embedding layer of source chars into args.cle_dim dimensions
                ...
                # TODO source_rnn as a bidirectional GRU with args.rnn_dim units, returning only the last state, summing opposite directions
                self.source_rnn = tf.keras.layers.Bidirectional(
                    tf.keras.layers.GRU(args.rnn_dim),
                    merge_mode='sum')

                # TODO target_embedding as an unmasked embedding layer of target chars into args.cle_dim dimensions
                self.target_embedding = tf.keras.layers.Embedding(
                    input_dim=num_source_chars,
                    output_dim=args.cle_dim,
                    mask_zero=False)
                # TODO target_rnn_cell as a GRUCell with args.rnn_dim units
                self.target_rnn_cell = tf.keras.layers.Bidirectional(
                    tf.keras.layers.GRU(args.rnn_dim),
                    merge_mode='sum')

                # TODO target_output_layer as a Dense layer into `num_target_chars`
                self.target_output_layer = tf.keras.layers.Dense(
                    num_target_chars, activation='softmax')

        # psa ->  pes
        # source_embedding -> target_embedding
        # emb(psa) -> source_rnn -> fixne velky neco co reprezentuje slovo
        # emb(pes) -> (kroky encoderu)

        # v attention bude return sequence true

        self._model = Model()

        self._optimizer = tf.optimizers.Adam()
        # xTODO: Define self._loss as SparseCategoricalCrossentropy which processes _logits_ instead of probabilities
        self._loss = tf.losses.SparseCategoricalCrossentropy()
        self._metrics_training = {
    ...
    def _append_eow(self, sequences):
        ...
        sequences_rev = tf.reverse_sequence(sequences,
                                            tf.reduce_sum(tf.cast(tf.not_equal(sequences, 0), tf.int32), axis=1), 1)
        sequences_rev_eow = tf.pad(
            sequences_rev, [[0, 0], [1, 0]], constant_values=MorphoDataset.Factor.EOW)
        return tf.reverse_sequence(sequences_rev_eow,
                                   tf.reduce_sum(tf.cast(tf.not_equal(sequences_rev_eow, 0), tf.int32), axis=1), 1)
    ...
    def train_batch(self, source_charseq_ids, source_charseqs, target_charseq_ids, target_charseqs):
        ...
        with tf.GradientTape() as tape:
            ...
            embedded_sequences = self._model.source_embeddings(source_charseqs)

            # TODO: Run self._model.source_rnn on the embedded sequences, returning outputs in `source_states`.
            source_states = self._model.source_rnn(embedded_sequences)
            ...
            class DecoderTraining(decoder.BaseDecoder):
                ...
                def initialize(self, layer_inputs, initial_state=None):
                    ...
                    bow = MorphoDataset.Factor.BOW
                    inputs = tf.fill(self.batch_size, bow)
                ...
                def step(self, time, inputs, states):
                    ...
                    next_inputs = self._target_embedding(self._targets[time])
                    # TODO: Define `finished` as True if `time`-th word from `self._targets` is EOW, False otherwise.
                    eow = MorphoDataset.Factor.EOW
                    finished = tf.equal(self._targets[time], eow)
                    # Again, no == or !=.
                    self._output_size = inputs.shape[1]
            ...
            tag_mask = tf.not_equal(self.targets, 0)
            loss = self._loss(targets, output_layer, tag_mask)
    ...
    def train_epoch(self, dataset, args):
        for batch in dataset.batches(args.batch_size):
            ...
            predictions = self.train_batch(
                ...
                batch[dataset.TAGS].charseq_ids,
                batch[dataset.TAGS].charseqs,
            )
    ...
    def predict_batch(self, source_charseq_ids, source_charseqs):
        ...
        class DecoderPrediction(decoder.BaseDecoder):
            ...
            def step(self, time, inputs, states):
                ...
                # `outputs, [states]`.
                outputs = self._model.target_rnn_cell(inputs, [states])
                ...
                output = tf.argmax(outputs, axis=1)
                ...
                finished = tf.equal(
                    self._targets, [bow for _ in self._targets])
                # Again, no == or !=.
                self._output_size = len(outputs)

                return outputs, states, next_inputs, finished

        predictions, _, _ = DecoderPrediction(maximum_iterations=tf.shape(source_charseqs)[1] + 10)(
            [self._model, source_states])
        return predictions
